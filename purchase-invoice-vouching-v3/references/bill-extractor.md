---
name: bill-extractor
description: Extracts a batch of vendor tax-invoices for vouching — vendor GSTIN, invoice no/date, every PR/PO reference (printed OR handwritten, plus any vendor-printed order number tagged separately), total, whether the buyer's stamp/signature is present, and every line (description, HSN/SAC, qty, rate, GST rate, amount), each value with a citation. Reuses the invoice-extraction-v5 cascade and gate. Read-only.
---
...
- `file_stems` — the invoice stems for this batch.
...
2. **`read_image` is the last resort, AFTER content.md, for exactly two things:**
   ...
   field), and (b) confirming the **buyer's stamp/signature** and any
   **handwritten PR** — visual marks OCR usually misses. Take one `read_image` of
   the relevant page, then stop.
   ...
   **DPI fallback.** `read_image` renders the page at its **default ~150 DPI**. A
   ...
   72-DPI render is coarser but usually succeeds and is still enough to read a stamp
   or a handwritten PR. Only if the 72-DPI retry also fails do you record that
   field/stamp as `unclear`/unverified — never invent it.
...
3. **Buyer (client) stamp / signature present?** The stamp that matters is the
   **buyer's** — the "Bill To" party (the client society / association / company)
   applies a **round rubber stamp** and/or a handwritten sign-off when it receives
   the bill. This is **not** the vendor's own "Authorised Signatory" seal or
   letterhead stamp — a vendor seal does **not** count. So look specifically for a
   stamp/sign-off bearing the **buyer's (Bill-To) name** — the same party whose
   `buyer_gstin` you captured. Look in the text layer first; if not found there, take
   **one** `read_image` of the page and look for the **buyer's** round stamp /
   signature. Emit `buyer_stamp`: `{present: "yes"|"no"|"unclear", page,
   source_text}` — `present:"yes"` **only** when the stamp is the buyer's;
   `source_text` = the buyer-name text inside the stamp if legible; `"unclear"` when
   a stamp is visible but you can't confirm it is the buyer's (e.g. it reads as the
   vendor's, or is illegible).
4. **Line items always in scope.** For every printed line, extract: `description`,
...
5. **Totals for reconciliation:** `taxable_total`, `tax_total` (and the CGST/SGST/
  ...
  "invoices": [
    {
      ...
      "buyer_stamp": {"present": "yes", "page": 1, "source_text": "Prestige Lakeside Habitat Home Owners Association"},
      "lines": [
        {"line_no": 1,
...
{"status": "ok", "agent": "bill_extractor_v3", "batch_index": 0,
 ...
 "handwritten_refs": 2, "stamp_unclear": 0, "no_hsn_invoices": 1,
...
- Capture **`vendor_gstin` (seller block) AND `buyer_gstin` (Bill-To block)** kept
  apart by section, **all** `references[]` (tagged buyer_pr_po vs vendor_ref), the
  **buyer's** `buyer_stamp` (a vendor seal does not count), and every line — with
  citations. Handwritten ref or stamp → one `read_image` of that page before concluding.
