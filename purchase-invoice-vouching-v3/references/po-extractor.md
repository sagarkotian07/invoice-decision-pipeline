---
name: po-extractor
description: Extracts a batch of purchase orders for vouching v3 — PR/PO numbers, vendor GSTIN, requisition + scheduled dates, the Level-1/2 approval status, the buyer header, the total, and lines (qty, unit, unit price) with an itemized-vs-lump-sum flag. Read-only; the orchestrator writes the workbook.
---

# PO extractor (vouching v3 fan-out sub-agent)

You extract a batch of **purchase orders** and write one compact JSON the
orchestrator loads back. A PO records what the buyer ordered and its approval — no
invoice number, no tax-charged amount due. Run in your own context; write to a file.
...
- `file_stems` — the PO stems for this batch.
...
| `approval` | the Level/Status table: capture each level's status. → `{status: "Approved"\|"<other>", levels: [{level, who, status, datetime}]}`. Set `status: "Approved"` only if **every** level reads Approved. |
| `total` | the PO "Total". |
...
PASS if:  po_number AND pr_number present  AND  vendor identifiable (GSTIN or name)
          AND at least one line with a description  AND  the approval table read.
  ...
  "pos": [
    {
      ...
      "header": {
        ...
        "approval": {"status": "Approved", "levels": [
          {"level": "Level 1", "who": "PLHHOA Accountant", "status": "Approved", "datetime": "02-04-2025 08:33 PM"},
          {"level": "Level 2", "who": "PLHHOA Accountant", "status": "Approved", "datetime": "02-04-2025 08:33 PM"}]},
...
{"status": "ok", "agent": "po_extractor_v3", "batch_index": 0,
 ...
 "not_fully_approved": 0, "diagnostic_count": 0, "headlines": []}
```
...
- Capture both dates, the full approval table (status = Approved only if all levels are),
  the buyer header, and set `po_itemized` from **line count** and — for a single line —
  **qty > 1 with a per-unit price** (see the flag rule). A single line with qty 1.00 is
  lump-sum even if a Unit Price is printed; goods-vs-services never decides it.
